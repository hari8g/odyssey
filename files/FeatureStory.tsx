...
export function FeatureStory() {
  ...
  useEffect(() => {
    if (!id) return
    Promise.all([
      api.invoke('ux:getFeatureStory', { featureId: parseInt(id) }),
      api.invoke('cycle:list'),
    ]).then(([s, runs]: [any, any[]]) => {
      setStory(s)
      const linked = runs.find((r: any) => r.feature_node_id === parseInt(id!))
                ...
                <StageRow key={stage.id} stageId={stage.id} state={state}
                  ...
                  {isWaiting && (
                    <WaitBlock reason={run?.error ?? undefined}
                      ...
                      onSimulate={() => api.invoke(`cycle:simulate${stage.id === 'SIGNALS' ? 'Signals' : stage.id === 'BUILD' ? 'CI' : 'Kpi'}`, { runId: run.id, drift: 0.9 })}
                    />
                  )}
                  {isGate && (
