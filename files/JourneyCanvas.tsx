...
import { Card, Badge, GateSeal, EmptyState, StatTile } from '../../design/primitives'
...
export function JourneyCanvas() {
  ...
  const { board, contexts, painPoints, signalCount } = useUXStore()
  ...
  function onStartInitiative(ppId?: number) {
    // Navigate to cycle runner in start-new mode, pre-seeding pain point
                  ...
                  {verb === 'LISTEN'
                    ...
                    : items.map(item => (
                        <InitiativeCard key={item.id} item={item}
                          onClick={() => navigate(`/feature/${item.featureId ?? item.id}`)} />
                      ))
                  }
                </div>
              </div>
