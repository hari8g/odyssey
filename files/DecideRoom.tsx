...
function PortfolioGate({ runId }: { runId: number }) {
  ...
  useEffect(() => {
    api.invoke('cycle:get', { runId }).then((run: any) => {
      Promise.all([
        run.packet_id ? api.invoke('ux:getGraphNode', { nodeId: run.packet_id }) : null,
        api.invoke('aep:getHypotheses'),
        run.feature_node_id
          ? Promise.resolve({ id: run.feature_node_id })
          : api.invoke('ux:getSuggestedFeature', { briefId: run.brief_id }),
      ]).then(([packet, hyps, feat]: [any, any[], any]) => {
        setData({ run, packet: packet ? JSON.parse(packet.description) : null, hyps })
        if (feat?.id) setFeatureId(feat.id)
      })
    })
  }, [runId])
  ...
  async function submit() {
    ...
    setSubmitting(true)
    try {
      await api.invoke('cycle:portfolioGate', {
        runId,
        input: { decision, approvedByRole: role, rationale, featureNodeId: featureId },
      })
...
function ReleaseGate({ runId }: { runId: number }) {
  ...
  useEffect(() => {
    api.invoke('cycle:get', { runId }).then((run: any) => {
      if (!run.readiness_report_id) return
      api.invoke('ux:getGraphNode', { nodeId: run.readiness_report_id }).then((node: any) => {
        setData({ run, report: JSON.parse(node.description) })
      })
    })
  }, [runId])
  ...
  async function sign() {
    ...
    setSubmitting(true)
    try {
      await api.invoke('cycle:signRelease', { runId, role, rationale })
      push({ message: `${role} signed — checking if all required signatures are present`, status: 'ok' })
      // Reload to reflect new state
      const run = await api.invoke('cycle:get', { runId })
      if (run.current_stage === 'ROLLOUT') {
        push({ message: 'All signatures complete — rollout starting', status: 'ok' })
        navigate(`/feature/${run.feature_node_id}`)
      }
    } catch (e: any) {
