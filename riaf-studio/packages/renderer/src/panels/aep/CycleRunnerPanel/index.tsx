...
export function CycleRunnerPanel() {
  ...
  }, [activeRunId, setRuns, setTimeline])
  ...
  useEffect(() => {
    ...
    void eAPI.cycleTimeline?.(activeRunId).then((tl: unknown) => {
  ...
  const startCycle = async () => {
    ...
    setError(null)
    try {
      const res = await eAPI.cycleStart?.({ label: label.trim() || 'Cycle', mode })
  ...
  const portfolioAdmit = async () => {
    if (!activeRunId || !activeRun?.feature_node_id) return
    setBusy(true)
    setError(null)
    try {
      const res = await eAPI.cyclePortfolioGate?.(activeRunId, {
        ...
        rationale: 'Admitted via Cycle Runner',
  ...
  return (
    <div className="relative flex flex-col h-full overflow-hidden">
      ...
      {error && (
        <div className="px-4 py-2 text-xs text-red-400 border-b border-border bg-red-400/5">
          {error}
        </div>
      )}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
          {!activeRun && (
            <p className="text-sm text-gray-500">
              Start a new cycle (demo mode recommended) to walk the full
              customer→learning sequence.
            </p>
          )}
